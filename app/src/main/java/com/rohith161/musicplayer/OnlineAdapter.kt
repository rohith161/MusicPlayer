...
class OnlineAdapter(private val onClick: (OnlineTrack) -> Unit) : RecyclerView.Adapter<OnlineAdapter.ViewHolder>() {
    ...
    fun setPlaylistMode(value: Boolean) { playlistMode = value; notifyDataSetChanged() }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_online, parent, false))
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: OnlineTrack) {
            ...
            action.setImageResource(when { playlistMode -> android.R.drawable.ic_menu_delete; value == 100 -> android.R.drawable.ic_menu_save; else -> android.R.drawable.ic_menu_more })
            action.contentDescription = if (playlistMode) "Remove from playlist" else "Song actions"
